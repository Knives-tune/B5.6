...
# поле игры из 3х частей : карта с кораблями игрока, радар с ходами игрока, поле с "весом" клеток для ИИ
...
class Field(object):
    ...
    # когда корабль уничтожен  все клетки вокруг помечаются сыгранными (Cell.miss_cell)
    ...
    def recalculate_weight_map(self, available_ships):
        ...
        # Если есть раненый корабль - клетки выше, ниже и по бокам
        # по диагоналям от раненой клетки ничего не может быть - там нули
        for x in range(self.size):
            for y in range(self.size):
        ...
        # Если там уничтоженый корабль, раненый или клетка с промахом - там 0
...
class Game(object):
    ...
    # старт игры , назначение текущего и следующего игрока
    def start_game(self):
    ...
    def status_check(self):
        ...
        if self.status == 'prepare' and len(self.players) >= 2:
            ...
            self.start_game()
            return True
        # статус game over если у следующего игрока осталось 0 кораблей.
    ...
    def ships_setup(self, player):
        for ship_size in Game.ships_rules:
            # задаем количество попыток при выставлении кораблей случайным образом, для того чтоюы не запустить бесконечный цикл
            retry_count = 30
            ...
            while True:
                ...
                # если корабль помещается на заданной позиции -добавляется корабль
                ...
                # если корабль не поместился - пишетчто позиция неправильная

                player.message.append('Неправильная позиция!')
                retry_count -= 1
                if retry_count < 0:
                    # после заданного количества неудачных попыток - обнуляет карту игрока
                    # убирает все корабли и начинаем расстановку по новой
...
class Player(object):
    ...
    # Ход игрока - либо расстановка кораблей (input_type == "ship_setup")
...
if __name__ == '__main__':
    ...
    # основные параметры игроков
    players = []
    players.append(Player(name='человек', is_ai=False, auto_ship=True, skill=1))
    players.append(Player(name='Мегатрон', is_ai=True, auto_ship=True, skill=1))
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            if shot_result == 'miss':
                ...
                game.next_player.message.append('Твой ход {}!'.format(game.next_player.name))
                game.switch_players()
                continue
